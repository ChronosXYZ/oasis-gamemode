...
{
PlayerOnFireController::PlayerOnFireController(IPlayerPool* playerPool, std::shared_ptr<dp::event_bus> bus)
	: bus(bus)
	, playerPool(playerPool)
	, roundEndRegistration(bus->register_handler<Utils::Events::RoundEndEvent>(this, &PlayerOnFireController::onRoundEnd))
{
...
void PlayerOnFireController::onPlayerDeath(IPlayer& player, IPlayer* killer, int reason)
{
	...
	if (kills == 6)
	{
		this->bus->fire_event(Utils::Events::PlayerOnFireEvent { .player = *killer, .lastKillee = player, .mode = killerExt->getMode() });
	}
	this->playersOnFire[killerExt->getMode()].emplace(killer);
}

void PlayerOnFireController::onPlayerDisconnect(IPlayer& player, PeerDisconnectReason reason)
{
