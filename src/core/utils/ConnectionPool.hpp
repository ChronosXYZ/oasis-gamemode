	...
	connection_pool(const connection_options& options)
		: connection_pool(
			std::format(
				...
				options.dbname, options.user, options.password,
				options.hostaddr, options.port),
			options.connections_count)
	{
	}

	connection_pool(const std::string& connection_string,
...
struct basic_connection final
{
	...
	~basic_connection() { pool.return_connection(manager); }
...
struct query_manager
{
	...
	template <typename... Args> pqxx::result exec_prepared(Args&&... args);
...
struct query
{
	...
	operator std::string() const { return { str.begin(), str.end() }; }
	...
	template <typename... Args> pqxx::result operator()(Args&&... args)
	{
	...
	template <typename... Args> pqxx::result exec(Args&&... args)
	{
		if (!manager.has_value())
			throw std::runtime_error("attempt to execute a query without "
...
struct basic_transaction
{
	...
	{
		const auto query_id
			= std::format("{:X}", std::hash<std::string_view>()(q));
	...
	template <typename... Queries> void prepare(Queries&&... queries)
	{
